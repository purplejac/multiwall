Puppet::Functions.create_function(:'multiwall::setup_ct_rules') do
  ...
  def setup_ct_rules(params)
    ...
    %w[ctorigdst ctrepldst ctorigsrc ctreplsrc].each do |addrparam|
      ...
      ip_arg = (params['source'].nil? and params['destination'].nil?) ? ' ip ' : ' '

      nft_command.push("ct #{traffic_type}#{ip_arg}#{ct_addr_dir} #{p_value}")
    end

    %w[ctorigdstport ctorigsrcport ctrepldstport ctreplsrcport].each do |addrparam|
      next if params[addrparam].nil?
